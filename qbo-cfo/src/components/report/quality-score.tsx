...
const BAND_STYLE: Record<string, { bar: string; badge: 'positive' | 'info' | 'warning' | 'negative' }> = {
  ...
  unknown: { bar: 'bg-surface-muted', badge: 'warning' },
};
...
export function QualityScoreCard({
  ...
  // A report stored before scoring existed has no score. Showing "0/100" would
  // read as a damning verdict on the bookkeeping rather than a missing field.
  const scored = score.band !== 'unknown';

  if (compact) {
    return (
      <span className="inline-flex items-center gap-2">
        <Badge variant={style.badge}>{scored ? `${score.score}/100 · ${score.bandLabel}` : score.bandLabel}</Badge>
      </span>
    );
  }
      ...
      <CardContent>
        {scored ? (
          ...
          <p className="text-sm text-ink-muted">
            This report was generated before the confidence score existed, so it has no score.
            Regenerating it computes one.
          </p>
        )}
        ...
        {!scored ? null : score.deductions.length === 0 ? (
